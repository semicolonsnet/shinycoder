...
if(exists("transcripts") == FALSE) {
  transcripts <<- tibble(ID = character(), participant = character(), HTMLfile = character())
  #transcripts <<- add_row(transcripts, ID = "sample", participant = "Sample Participant", HTMLfile = "sample.html")
}

if(exists("coded_text") == FALSE) {
  coded_text <<- tibble(participant = character(), textid = character(), code = character(), text = character())
}

if(exists("codes") == FALSE) {
  codes <<- list("Choice 1" = 1, "Choice 2" = 2, "Choice 3" = 3)
}

if(exists("participants") == FALSE) {
  participants <<- c("No participant added")
}

active_transcript_text <<- character(0)
coded_excerpt <<- character(0)

## Dummy Transcripts Variable - Remove once we can load from list
currentID <<- "sample"
...
ui <- bootstrapPage(
  ...
  navbarPage("Shinycoder", id = "tabs",
    ...
    tabPanel("Transcripts", 
      column(4,
        fileInput("importedFile", "Choose MD file", accept = c("text/plain", "text/markdown","text/md", ".md", ".txt")),
        radioButtons("transcriptParticipant", label = h3("Participants"),
    ...
    tabPanel("Code Transcript",
      ...
      fluidRow(
        ...
        column(4,
          ...
          radioButtons("code_radio", label = h3("Codes"),
            choices = codes,
            selected = 1),
